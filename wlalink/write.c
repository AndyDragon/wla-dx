...
int _ad1_write_bank_header(FILE *f, unsigned short bank, unsigned short size) {

  fprintf(f, "%c%c", bank & 0xFF, (bank >> 8) & 0xFF);
  fprintf(f, "%c%c", size & 0xFF, (size >> 8) & 0xFF);
...
int write_rom_file(char *outname) {
  ...
  /* AD1 PROG file output mode */
  else if (g_output_type == OUTPUT_TYPE_AD1_PROG) {
    if (g_banksize > 0x10000) {
      fprintf(stderr, "WRITE_PROG_FILE: The ROM bank size was greater than 0x10000 ($%x).\n", g_banksize);
      fclose(f);
      return FAILED;
    }
    /* write bank by bank and bank header sections */
    for (j = 0; j < g_rombanks; j++) {
      e = 0;
      for (i = 0; i < g_banksize; i++) {
        if (g_rom_usage[j * g_banksize + i]) {
          e = i + 1;
        }
      }
      if (e) {
        if (_ad1_write_bank_header(f, j, e) == FAILED) {
          fclose(f);
          return FAILED;
        }
        fwrite(g_rom + j * g_banksize, 1, e, f);
      }
    }
  }
